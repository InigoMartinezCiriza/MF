...
class NystroemFeaturesSampler():
    ...
    def transform(self, X_prime: np.ndarray) -> np.ndarray:
        ...
        if self._X_reduced is None or self._reduced_kernel_matrix is None:
            raise ValueError("Fit the model before transforming data.")

        # Compute kernel between input and sampled instances
        K_X_prime_reduced = self._kernel(X_prime, self._X_reduced)

        # Compute Nystroem features
        X_prime_nystroem = K_X_prime_reduced @ self._sqrtm_pinv_reduced_kernel_matrix
